...
type ILBuilder(semanticAnalysisResult) =
    ...
    member x.BuildClass (program : Ast.Program) =
        ...
        let processCast fromType toType =
            match toType with
            | Ast.String ->
                ILOpCode.CallClr((typeof<Object>).GetMethod("ToString", Array.empty))
            | Ast.Int ->
                ILOpCode.Conv_i4
            | Ast.Double ->
                ILOpCode.Conv_r8
        ...
        let builtInMethods = [
            ...
            };
            {
                ...
                Locals = [];
                Body = [ Ldarg(0s)
                         processCast Ast.Int Ast.String
                         Ret ];
            };
            {
                Name = "dtostr";
                ...
                Locals = [];
                Body = [ Ldarg(0s)
                         processCast Ast.Double Ast.String
                         Ret ];
            };
            {
                Name = "itod";
                ...
                Locals = [];
                Body = [ Ldarg(0s)
                         processCast Ast.Int Ast.Double
                         Ret ];
            };
            {
                Name = "dtoi";
                ...
                Locals = [];
                Body = [ Ldarg(0s)
                         processCast Ast.Double Ast.Int
                         Ret ];
            };
        ]
