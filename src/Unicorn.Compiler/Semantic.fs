...
type FunctionCalls(program) as self =
    inherit Dictionary<Ast.Identifier, FunctionTableEntry>()

type ExpressionTypeTable(program, functionTable : FunctionTable, functionCalls: FunctionCalls, symbolTable : SymbolTable) as self =
    ...
    and scanExpression expression =
        ...
        let expressionType =
            ...
            | Ast.FunctionCallExpression(i, a, _) -> // e.g. myFunc(1, "a")
                ...
                functionCalls.[i] <- calledFunction
...
let analyze program =
    try
        ...
        let functionCalls = FunctionCalls()
        ...
        let expressionTypes = ExpressionTypeTable(program, functionTable, functionCalls, symbolTable)
